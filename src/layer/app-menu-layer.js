var AppMenuLayer = cc.Layer.extend({
	...
	ctor: function(){
		...
		var move = cc.moveTo(5, cc.p(0, -10)).easing(cc.easeElasticOut());
		...
		var actionTo = cc.moveTo(1, cc.p(250, winsize.height-150)).easing(cc.easeElasticOut());
		var sequence = cc.sequence(
				actionTo,
				cc.callFunc(function (logo) {
					var shaking = cc.moveTo(1, cc.p(250, winsize.height-250)).easing(cc.easeElasticIn());
					var shakingBack = cc.moveTo(1, cc.p(250, winsize.height-140)).easing(cc.easeElasticOut());
					var shakingSeq = cc.sequence(shaking, shakingBack);
					var shakingSeq = cc.sequence(shaking,shakingBack);
		...
		var seq = cc.sequence(
				cc.moveTo(2, cc.p(playBtnPosX, playBtnPosY)).easing(cc.easeElasticInOut(0.8)),
				cc.callFunc(function(playBtn){
					var shaking = cc.moveTo(1, cc.p(playBtnPosX, playBtnPosY)).easing(cc.easeIn(2.0));
					var shakingBack = cc.moveTo(1, cc.p(playBtnPosX, playBtnPosY-10)).easing(cc.easeOut(2.0));
					var shakingSeq = cc.sequence(shaking, shakingBack);
					var shakingSeq = cc.sequence(shaking, shakingBack);
		...
		var actionTo = cc.moveTo(2, cc.p(winsize.width-200, winsize.height-220)).easing(cc.easeElasticOut());
		var seq = cc.sequence(
				actionTo,
				cc.callFunc(function(storeBtn){
					var shaking = cc.moveTo(2, cc.p(winsize.width-205, winsize.height-220)).easing(cc.easeBackInOut());
					var shakingBack = cc.moveTo(2, cc.p(winsize.width-195, winsize.height-220)).easing(cc.easeBackInOut());
					var shakingSeq = cc.sequence(shaking, shakingBack);
		...
		var actionTo = cc.moveTo(2, cc.p(winsize.width-200, winsize.height-300)).easing(cc.easeElasticOut());
		var seq = cc.sequence(
				actionTo,
				cc.callFunc(function(setBtn){
					var shaking = cc.moveTo(2, cc.p(winsize.width-205, winsize.height-300)).easing(cc.easeBackInOut());
					var shakingBack = cc.moveTo(2, cc.p(winsize.width-195, winsize.height-300)).easing(cc.easeBackInOut());
					var shakingSeq = cc.sequence(cc.delayTime(0.3),shaking, shakingBack);
		...
		var actionTo = cc.moveTo(2, cc.p(winsize.width-200, winsize.height-375)).easing(cc.easeElasticOut());
		var seq = cc.sequence(
				actionTo,
				cc.callFunc(function(aboutBtn){
					var shaking = cc.moveTo(2, cc.p(winsize.width-205, winsize.height-375)).easing(cc.easeBackInOut());
					var shakingBack = cc.moveTo(2, cc.p(winsize.width-195, winsize.height-375)).easing(cc.easeBackInOut());
					var shakingSeq = cc.sequence(cc.delayTime(0.2), shaking, shakingBack);
		...
		var seq = cc.sequence(moveTo, cc.callFunc(function(panda){
	...
	onSet: function () {
		...
		this.backBtn.runAction(cc.moveTo(1, cc.p(winsize.width/2-100, winsize.height/2-160)).easing(cc.easeElasticOut()));
		...
		var toggler = new cc.MenuItemToggle( on, off,
				function(that){
					...
					if(canAudioPlaying) {
						...
						cc.sys.localStorage.setItem("canAudioPlaying", 0);
					} else {
						canAudioPlaying = true;
						cc.sys.localStorage.setItem("canAudioPlaying", 1);
					}
				},this);
		...
		this.effect.runAction(cc.moveTo(1, cc.p(winsize.width/2-30, winsize.height/2+10)).easing(cc.easeElasticOut()));
		...
		var toggler = new cc.MenuItemToggle( onAudio, offAudio,
				function(that){
					...
					if(canMusicPlaying) {
						...
						cc.sys.localStorage.setItem("canMusicPlaying", 0);
						cc.audioEngine.pauseMusic();
					} else {
						canMusicPlaying = true;
						cc.sys.localStorage.setItem("canMusicPlaying", 1);
		...
		this.audio.runAction(cc.moveTo(1, cc.p(winsize.width/2-30, winsize.height/2-65)).easing(cc.easeElasticOut()));
		...
		var toggler = new cc.MenuItemToggle( onDiff, offDiff,
				function(that){
					...
					if(diffDeg == 0) {
						diffDeg = 1;
						cc.sys.localStorage.setItem("diffDeg", 1);
					} else {
						diffDeg = 0;
						cc.sys.localStorage.setItem("diffDeg", 0);
					}
		...
		this.diff.runAction(cc.moveTo(1, cc.p(winsize.width/2-30, winsize.height/2-140)).easing(cc.easeElasticOut()));
...
//		this.logo.runAction(sequence);
		...
		this.backBtn.setScale(0.8);
		this.backBtn.runAction(cc.moveTo(1, cc.p(winsize.width/2-100, winsize.height/2-190)).easing(cc.easeElasticOut()));
		this.addChild(this.backBtn, 6);
	},
	...
	backToMenu: function() {
		...
		this.board.runAction(cc.moveTo(1, cc.p(-250, winsize.height/2)).easing(cc.easeElasticInOut(0.45)));
		if(this.effect)
